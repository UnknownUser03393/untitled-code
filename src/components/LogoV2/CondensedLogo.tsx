...
export function CondensedLogo() {
  ...
  const modelDisplayName = providerPresentation?.model ?? renderModelSetting(model);
  const {
    version,
    cwd,
    billingType,
  ...
  const {
    ...
    shouldSplit: false,
    truncatedModel: truncate(modelDisplayName + effortSuffix, textWidth),
    truncatedBilling: ''
  ...
  if ($[11] !== shouldSplit || $[12] !== truncatedBilling || $[13] !== truncatedModel) {
    t7 = !truncatedBilling ? <Text dimColor={true}>{truncatedModel}</Text> : shouldSplit ? <><Text dimColor={true}>{truncatedModel}</Text><Text dimColor={true}>{truncatedBilling}</Text></> : <Text dimColor={true}>{truncatedModel} · {truncatedBilling}</Text>;
    $[11] = shouldSplit;
    $[12] = truncatedBilling;
    $[13] = truncatedModel;
    $[14] = t7;
